...
export class ProgressService {
  static async updateProgress(userId: number, videoId: number, last_position: number, is_completed: boolean = false) {
    ...
    if (existingRow) {
      // If already completed, don't revert to uncompleted unless specifically asked
      // However we want to update the last position
    ...
    // After updating, check if the session is finished for this subject
