...
export class SubjectService {
  static async getSubjectTree(subjectId: string | number, userId: string | number) {
    ...
    // Construct the global sequence to handle locking
    // Prerequisite: Previous video must be completed
    let isPreviousCompleted = true; // First video is always unlocked
  ...
  static async getVideoDetails(videoId: string | number, userId: string | number) {
    ...
    // Get all videos in this subject for navigation
    ...
    const allVideos = await db('videos')
      ...
      .orderBy('section_id', 'asc') // simplified for now, ideally correct global order
      .orderBy('order_index', 'asc');
